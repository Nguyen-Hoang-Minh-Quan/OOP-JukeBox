...
import track_library as TrackLibary
import tkinter.scrolledtext as tksrcolled
...
class CreateTrackList:
    def __init__(self, window):
        ...
        self.text_area = tksrcolled.ScrolledText(self.window, width=85, height=15)
    ...
    def add_track(self):
        ...
        track_name = TrackLibary.get_name(track_number)
        ...
        if track_name is not None:
            ...
            self.display_message("Error: Wrong number, please try again.")
    ...
    def play_playlist(self):
        ...
        if not self.playlist: # Check if the playlist is empty
            self.display_message("Error: Playlist is empty, it can not play.Please, add tracks first.")
            return
        ...
        for track_number in self.playlist:
            ...
            if TrackLibary.get_name(track_number) is not None:
                TrackLibary.increment_play_count(track_number) # Increment the play count
                tracks_played += 1
            else:
        ...
        if tracks_played > 0:
         ...
         self.display_message("Not valid tracks been found to play.")
    ...
    def update_playlist_display(self):
        ...
        for track_number in self.playlist:
            track_name = TrackLibary.get_name(track_number)
            if track_name is not None:
                artist = TrackLibary.get_artist(track_number)
                rating = TrackLibary.get_rating(track_number)
                play_count = TrackLibary.get_play_count(track_number)
